...
namespace spi
{
    ...
    public:
        ...
        {
            _log(logging::Debug) << "Setting up logging at " << logRoot << std::endl;
            if (bfs::exists(logRoot) && !bfs::is_directory(logRoot)) {
                _log(logging::Error) << "Setup failure: " << logRoot
                                            << " already exists and is not a directory" << std::endl;
                return false;
            }
            if (!bfs::exists(logRoot) && !bfs::create_directories(logRoot)) {
                _log(logging::Error) << "Setup failure: unable to create directory " << logRoot << std::endl;
                return false;
            }
            _log(logging::Info) << "LogManager started successfully" << std::endl;
            _logRoot = logRoot;
            return true;
        }

    private:
        logging::Logger _log{"log-manager", logging::Debug};
        bfs::path _logRoot;
    };
}
